...
jatemplate="$swd""CSF/jobarray.template"
program="$swd""singularity_rip.sh"
...
# awk -F '-od=' '{print "mkdir -p " $2}' $inputsfile > dirsA.sh        # Gets all the paths from inputs file
# awk -F '-pd=' '{print "mkdir -p " $1}' dirsA.sh > dirs.sh        # Gets all the paths from inputs file
...
# rm dirsA.sh dirs.sh deduplicated_dirs.sh                                     # Removes temp files
