...
def frontogenesis3D(ncfile,time):
    if type(ncfile) is Dataset:
        ...
        print(f"ncfile should be a netCDF4.Dataset or a string with a path to a dataset. {type(ncfile)} is not valid.")
        return 1
    # Fetch the fields we need
    p = to_np(getvar(ncFile, "pressure",timeidx=time) * 100)    # The 100 converts hPa to Pa
    ...
    F3D = 1.08e9 * (1 / theta_gradient) * (zonal_gradient + meridional_gradient + vertical_gradient)    # The 1.08e9 converts from [K/m/s] to [K/100km/3h]
    return F3D
